  ...
  return (
    ...
    >
      <div className="mx-auto flex h-16 max-w-[1366px] items-center justify-between gap-4 px-4 sm:h-[72px] sm:px-6">
        ...
        >
          <ul className="flex items-center gap-1 text-sm font-bold tracking-[0.04em]">
            {NAV.map((item) => (
              <li key={item.href}>
                <Link
                  href={item.href}
                  className="inline-flex items-center rounded-lg px-3 py-2 text-ink/75 transition hover:bg-ink/[0.04] hover:text-ink"
        ...
        {/* Desktop right side: compact search + CTA */}
        <div className="hidden items-center gap-2 md:flex">
          <form
            action="/search"
            method="get"
            role="search"
            className="flex h-10 items-center gap-2 rounded-xl border border-ink/15 bg-white px-3 transition focus-within:border-primary focus-within:shadow-[0_0_0_3px_rgba(21,86,238,0.12)]"
          ...
          >
            ...
            <input
              ...
              autoComplete="off"
              className="h-full w-40 bg-transparent text-sm text-ink outline-none placeholder:text-ink/40 lg:w-56"
